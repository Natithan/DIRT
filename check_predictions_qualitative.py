...
def main(_):
    ...
    for trained_model_path in (best_val_run_path, latest_run_path):
        ...
        for name, dataset in zip(('Train', 'Test', 'Val'), (train_dataset, test_dataset, val_dataset)):
            ...
            inputs = [model.token_indexer.decode(instance.fields['input_ids'].array) for instance in instances]
            prediction = model.forward_on_instances(
                instances)
            predicted_words = [model.token_indexer.decode(prediction[batch_sample]['vocab_logits'].argmax(1)) for batch_sample in
                               range(len(prediction))]  # TODO avoid my model just outputting 'mask' all of the time ;)
