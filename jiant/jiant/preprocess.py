...
def get_tasks(args):
    ...
    scratch_path = scratch_path or data_path # set scratch path to data path if scratch path is empty
...
class ModelPreprocessingInterface(object):
    ...
    lm_boundary_token_fn: (list[str] -> list[str]):
        A function that applies the appropriate EOS/SOS/SEP/CLS tokens to a token sequence for
