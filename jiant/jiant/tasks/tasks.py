...
@register_task("cc", rel_path='CC')
class ConceptualCaptionsLMTask(Task):
    def __init__(self, path, max_seq_len, name, **kw):
        super().__init__(name, **kw)
        self.data_path = path
        self.max_seq_len = max_seq_len
    ...
    def load_data(self):
        """Process the dataset located at self.data_path"""
        pkl_path = Path(self.data_path, f'data.pkl')
        if not os.path.exists(pkl_path):
            pkl = DotMap()
            for split in ['train', 'val', 'test']:
                data = pd.read_csv(Path(self.data_path, f'{split}.tsv'), '\t', names=['captions', 'urls'])
                mixed_data = self.mix_data(data)
                images = mixed_data.urls.apply(self.array_from_url)
                pkl[split] = DotMap({'captions': mixed_data.captions, 'images': images, 'labels': mixed_data.labels})
            pickle.dump(pkl, open(pkl_path, 'wb'))
        self.data = pickle.load(open(pkl_path, 'rb'))
        self.train_data_text = self.data.train.captions
        self.val_data_text = self.data.val.captions
        self.test_data_text = self.data.test.captions
        self.sentences = self.train_data_text + self.val_data_text

    def get_split_text(self, split: str):
        """ Get split data , typically as list of columns. #TODO in own implementation, if not only text, rename in preprocessing

        Split should be one of 'train', 'val', or 'test'.
        """
        return self.data[f'{split}']

    def mix_data(self, data):
        """
        Return data where half of the captions are labeled true, and matched with the correct image,
        and half are labeled false and get a randomly assigned incorrect image
        """
        true_data = data.sample(math.ceil(len(data) / 2))
        other_data = data.drop(true_data.index).sample(frac=1)  # sample(frac=1) is same as permuting rows
        false_data = other_data.copy()
        false_data.urls = np.roll(other_data.urls, 1)
        true_data['labels'] = 1
        false_data['labels'] = 0
        return true_data.append(false_data).sample(frac=1).reset_index(drop=True)
    ...
    def get_num_examples(self, split_text):
        ...
        Overriden.
        """
        return len(split_text.captions)

    def array_from_url(self, url):
        """
        Returns a numpy array for the image at the url.
        If it cannot be found, returns None
        """
        try:
            return np.array(Image.open(BytesIO(requests.get(url).content)))  # TODO Nathan probably compress images
        except OSError:
            return None
    ...
    def process_split(
        ...
        def _make_instance(caption, image, label):
            d = {}
            d['caption'] = TextField(list(map(Token, caption.split(" "))), token_indexers=indexers)
            d['labels'] = LabelField(label, label_namespace='labels', skip_indexing=True)
            d['image'] = ArrayField(image)
            return Instance(d)

        instances = map(_make_instance, *(split.captions, split.images, split.labels))
        return instances  # lazy iterator
